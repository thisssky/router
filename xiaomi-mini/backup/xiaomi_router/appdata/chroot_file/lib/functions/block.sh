...
config_create_swap_fstab_entry() {
	...
	cat /tmp/fstab 2>/dev/null | grep -E -v "^$device[[:blank:]]" >>"$fstabnew"
...
config_create_mount_fstab_entry() {
	...
	lock /var/lock/fstab.lck
	fstabnew="$(mktemp -t '.fstab.XXXXXXXX')"
	cat /tmp/fstab 2>/dev/null | grep -E -v "^$device[[:blank:]]" | grep -v "$target" >>"$fstabnew"
...
#This function is to get if a partition is GPT reserved partition. SW should skip mount this partition.
proc_gpt_rsvd() {
